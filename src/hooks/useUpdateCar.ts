...
export default function useUpdateCar() {
  const { selectedCar, selectCar, setSelectedCarName, setSelectedCarColor } = useGarageStore(
    useShallow((state) => ({
      selectedCar: state.selectedCar,
      selectCar: state.selectCar,
      setSelectedCarName: state.setSelectedCarName,
      setSelectedCarColor: state.setSelectedCarColor,
    })),
  );
