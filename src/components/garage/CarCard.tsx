...
export default function CarCard({ car }: { car: Car }) {
  const { raceState, isRacing } = useGarageStore(
    useShallow((state) => ({
      raceState: state.raceStates[car.id],
      isRacing: state.raceStates[car.id]?.status === "driving" || state.raceStates[car.id]?.status === "starting",
    })),
  );
